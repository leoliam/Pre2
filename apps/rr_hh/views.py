...
def ReporteHorario(req):		
	return render(req, 'rr_hh/ReporteHorario.html')


def crear_reporte_horario(request,value,opcion,idempleado):
	empleado = get_object_or_404(Empleado, id=idempleado)
	servicio = get_object_or_404(Servicio, servi_nom=opcion)
	ingreso=''
	salida=''
	consulta = ServicioEmpleado.objects.filter(empleado=idempleado,servicio=value) 
	lista=[] 
	for carga in consulta:
		...
		for carga2 in consulta2:
			if int(carga2.hor_ing.strftime("%H")) >=12:
				ingreso=' PM'
			else:
				ingreso=' AM'
			if int(carga2.hor_sal.strftime("%H"))>=12:
				salida=' PM'
			else:
				salida=' AM'
			d={'turno':carga2.turno,'fecha':carga2.hor_fecha.strftime("%d/%m/%Y"),'ingreso':carga2.hor_ing.strftime("%I:%M")+ingreso,'salida':carga2.hor_sal.strftime("%I:%M")+salida}	
			lista.append(d)
	html = render_to_string("rr_hh/reportehorser.html",{'pagesize':'A4','empleado':empleado,'lista':lista, 'servicio':servicio}, context_instance=RequestContext(request))
...
def BuscarEmpleado(request):		
	if request.is_ajax():			
		...
		if seleccion=="0":
			...
			fields={'emp_nom','emp_ape','emp_tel',
			'emp_direccion','DNI','emp_fecnac'})
...
def CreatePersonal(request):	
	if request.method == 'POST':
		...
		if form.is_valid() :			
			...
			#empleado.avatar=request.POST['avatar']
...
def BuscarPersonal(request):		
	if request.is_ajax():			
		...
		if seleccion=="0":
			...
			empleado = Empleado.objects.filter(cargo__cargo_nom="Especialista")
...
def HomePersonal(req):
	tmpl_vars = {
		...
		'form': UserForm(),'form2':UserFormModi
	}
	...
	if request.method == 'POST':
		...
		if form.is_valid() :
			...
			if form.cleaned_data.get('hor_sal')<=form.cleaned_data.get('hor_ing'):
				response_data['error'] = "La hora de salida debe ser mayor a la hora de ingreso"
			else:
				if (form.cleaned_data.get('turno')== "M") and (form.cleaned_data.get('hor_sal').hour>13) :
					response_data['error'] = "El Turno Mañana solo puede durar hasta la 01:00 PM"
					return HttpResponse(json.dumps(response_data),content_type="application/json")
				elif (form.cleaned_data.get('turno')== "M") and (form.cleaned_data.get('hor_sal').hour==13) and (form.cleaned_data.get('hor_sal').minute>0):
					response_data['error'] = "El Turno Mañana solo puede durar hasta la 01:00 PM"
					return HttpResponse(json.dumps(response_data),content_type="application/json")
				elif (form.cleaned_data.get('turno')== "T") and (form.cleaned_data.get('hor_ing').hour<13):
					response_data['error'] = "El Turno Tarde solo puede Iniciar apartir la 01:00 PM"
					return HttpResponse(json.dumps(response_data),content_type="application/json")
				else :
